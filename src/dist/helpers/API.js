...
let baseUrl = `https://app-hrsei-api.herokuapp.com/api/fec2/hr-rfe`;
let endpoints = {
...
let GET_PRODUCTS = () => {
    return axios.get(`${baseUrl}${endpoints['GET_PRODUCTS']}`);
};

//Returns all product level information for a specified product id.
let GET_PRODUCT = (PRODUCT_ID) => {
    return axios.get(`${baseUrl}${endpoints['GET_PRODUCTS']}${PRODUCT_ID}`);
};
//Returns the all styles available for the given product.
let GET_PRODUCT_STYLES = (PRODUCT_ID) => {
    return axios.get(`${baseUrl}${endpoints['GET_PRODUCTS']}${PRODUCT_ID}/styles`);
};
//Returns the id's of products related to the product specified.
let GET_PRODUCT_RELATED = (PRODUCT_ID) => {
    return axios.get(`${baseUrl}${endpoints['GET_PRODUCTS']}${PRODUCT_ID}/related`);
};
...
let GET_REVIEWS = (PRODUCT_ID, PAGE=1, COUNT=5, SORT='newest') => {
    return axios.get(`${baseUrl}${endpoints['GET_REVIEWS']}?product_id=${PRODUCT_ID}&page=${PAGE}&count=${COUNT}&sort=${SORT}`);
};
...
let POST_REVIEWS = (BODY) => {
    return axios.post(`${baseUrl}${endpoints['POST_REVIEWS']}`, BODY);
};
...
let POST_QA_QUESTION_ANSWER = (QUESTION_ID, BODY) => {
    return axios.post(`${baseUrl}${endpoints['POST_QA_QUESTION']}${QUESTION_ID}/answers`, BODY);
}
//Updates a question to show it was found helpful.
let PUT_QA_QUESTION_HELPFUL = (QUESTION_ID) => {
    return axios.put(`${baseUrl}${endpoints['PUT_QA_QUESTION']}${QUESTION_ID}/helpful`);
}
//Updates a question to show it was reported. Note, this action does not delete the question, but the question will not be returned in the above GET request.
let PUT_QA_QUESTION_REPORT = (QUESTION_ID) => {
    return axios.put(`${baseUrl}${endpoints['PUT_QA_QUESTION']}${QUESTION_ID}/report`);
}
//Updates an answer to show it was found helpful.
let PUT_QA_ANSWER_HELPFUL = (ANSWER_ID) => {
    return axios.put(`${baseUrl}${endpoints['PUT_QA_ANSWER']}${ANSWER_ID}/helpful`);
};
//Updates an answer to show it has been reported. Note, this action does not delete the answer, but the answer will not be returned in the above GET request.
let PUT_QA_ANSWER_REPORT = (ANSWER_ID) => {
    return axios.put(`${baseUrl}${endpoints['PUT_QA_ANSWER']}${ANSWER_ID}/report`);
};

//Retrieves list of products added to the cart by a user.
let GET_CART = () => {
    return axios.get(`${baseUrl}${endpoints['GET_CART']}`);
};
...
let POST_INTERACTION = (BODY) => {
    return axios.post(`${baseUrl}${endpoints['POST_INTERACTIONS']}`, BODY);
}
export default {
    GET_PRODUCTS,
    GET_PRODUCT,
